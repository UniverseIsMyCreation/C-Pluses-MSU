...
mstring_excpeption::mstring_excpeption(const char * name_operator, int arg1, int arg2, int amount_args)throw()
{
...
mstring_excpeption::mstring_excpeption(const mstring_excpeption & obj_excp)throw()
{
...
mstring_excpeption & mstring_excpeption::operator=(const mstring_excpeption & obj_excp)throw()
{
    strcpy(err_operator, obj_excp.err_operator);
    arg1 = obj_excp.arg1;
    arg2 = obj_excp.arg2;
...
void mstring_excpeption::what() const throw()
{
    switch(amount_args)
    {
        case(1):
            cerr << "Exception: problem in " << err_operator << " incorrect parameter:" << arg1 << endl;
            break;
        case(2):
            cerr << "Exception: problem in " << err_operator << " incorrect parameter:" << arg1 << " " << arg2 << endl;
            break;
        default:
            break;
    }
...
char & mstring::operator[] (int i) const
{
    if (i >= length() || i < 0)
    {
        mstring_excpeption new_obj_excp("operator[]",i,EMPTY,AMOUNT_ARGS_1);
        throw new_obj_excp;
    }
    return m_string[i];
}
...
const mstring mstring::operator* (int n) const
{
    ...
    if (n < 0)
    {
        mstring_excpeption new_obj_excp("operator*",n,EMPTY,AMOUNT_ARGS_1);
        throw new_obj_excp;
    }
    for (int i = 0; i<n; i++)
    {
...
const mstring operator* (int n,const mstring & obj_str)
{
    ...
    if (n < 0)
    {
        mstring_excpeption new_obj_excp("operator*",n,mstring::EMPTY,mstring::AMOUNT_ARGS_1);
        throw new_obj_excp;
    }
    for (int i = 0; i<n; i++)
    {
...
void mstring::insert(char c,int i)//insert symbol on position(also 0)
{
    if (i < 0 || i > length())
    {
        mstring_excpeption new_obj_excp("insert",i,EMPTY,AMOUNT_ARGS_1);
        throw new_obj_excp;
    }
    char * new_str = new char[length() + 2];
    memcpy(new_str,m_string,length() + 1);
...
void mstring::del(int i)//delete symbol on position
{
    if (i < 0 || i >= length())
    {
        mstring_excpeption new_obj_excp("del",i,EMPTY,AMOUNT_ARGS_1);
        throw new_obj_excp;
    }
    step_left(i);
...
{
    if (i < 0 || j < 0 || j < i || j >= length() || i >= length())
    {
        mstring_excpeption new_obj_excp("del",i,j,AMOUNT_ARGS_2);
        throw new_obj_excp;
    }
    step_left(i,j-i+1);
