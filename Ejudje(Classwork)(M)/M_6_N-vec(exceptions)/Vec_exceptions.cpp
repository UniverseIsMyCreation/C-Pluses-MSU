...
double & Vec::operator[] (int n) const
{
    if (n < 0 || n >= len)
    {
        Vec_excpeption new_obj_excp("incorrect indexing: ",n,Vec::EMPTY,Vec::AMOUNT_AGRS_1);
        throw new_obj_excp;
    }
    return v[n];
}
...
const Vec Vec::operator+ (const Vec & obj_vec) const
{
    if (len != obj_vec.len)
    {
        Vec_excpeption new_obj_excp("addition of vectors of different lengths: ",len,obj_vec.len,Vec::AMOUNT_AGRS_2);
        throw new_obj_excp;
    }
    Vec new_vec(len);
    for (int i = 0; i < len; i++)
...
Vec::Vec(int length, double *coord)
{
    if (length < 0)
    {
        Vec_excpeption new_obj_excp("length error",Vec::EMPTY,Vec::EMPTY,Vec::NOTHING);
        throw new_obj_excp;
    }
    len = length;   
    v = new double[len];
...
void Vec::set(double arg, int coord)
{
    ...
    {
        Vec_excpeption new_obj_excp("coordinate error in set()",Vec::EMPTY,Vec::EMPTY,Vec::NOTHING);
        throw new_obj_excp;
    }
    v[coord] = arg;
}
...
double Vec::get(int coord) const
{
    if (coord < 0 || coord >= len)
    {
        Vec_excpeption new_obj_excp("coordinate error in get()",Vec::EMPTY,Vec::EMPTY,Vec::NOTHING);
        throw new_obj_excp;
    }
    return v[coord];
}
